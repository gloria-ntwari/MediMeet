...
export default function DoctorLogin() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            console.log('Attempting doctor login with:', formData.email);
            const user = await authService.login(formData);
            console.log('Login response:', user);

            // Verify if the user is a doctor
            if (user.role !== 'Doctor') {
                setError('Access denied. Please use doctor credentials.');
                setIsLoading(false);
                return;
            }

            // Add a log to verify doctor login
            console.log('Doctor login successful, storing credentials and navigating to dashboard');

            // Explicitly store credentials even though authService should also do it
            localStorage.setItem('userEmail', formData.email);
            localStorage.setItem('userPassword', formData.password);
            localStorage.setItem('user', JSON.stringify(user));
            localStorage.setItem('userRole', user.role);

            // Check if token was received and store it
            if (user?.token) {
                localStorage.setItem('jwt_token', user.token);
                console.log('JWT token stored');
            } else {
                console.log('No JWT token received, using basic auth');
            }

            // Force a direct login attempt to verify credentials
            try {
                const authCheck = await authService.checkAuthStatus();
                console.log('Auth check after login:', authCheck);

                if (!authCheck.authenticated || (authCheck.userFound && authCheck.role !== 'Doctor')) {
                    console.warn('Auth check failed after login, but proceeding anyway');
                }
            } catch (error) {
